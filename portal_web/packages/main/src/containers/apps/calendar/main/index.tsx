...
const Main: FC = () => {
  const events: IEvent[] = useAppSelector((state) => state.events.eventSources);
  ...
  const eventRender = (info: any) => {
    ...
    if (view.type === "listMonth" || view.type === "listWeek") {
      ...
      el.querySelector(".fc-list-event-time").style.borderColor = borderColor;
      el.querySelector(".fc-list-event-title").style.borderColor = borderColor;
  ...
  const handleEventClick = (info: any): void => {
    const { id } = info.event;
    const { category } = info.event.source.internalEventSource.extendedProps;
    const bg = info.event.source.internalEventSource.ui.borderColor;
    const { meta } = info.event.source.internalEventSource;
    const selectedEvent = meta.find((item: { id: string }) => item.id === id);
